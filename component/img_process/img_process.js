...
imgprocessRouter.get('/', (req, res) => {
    ...
    if (req.session.user) {
        pool.query('SELECT * FROM table_oss WHERE compliance_check = \'No\'', (err, result) => {
...
imgprocessRouter.post('/update', (req, res) => {
    if (req.session.user) {
        ...
        pool.query('UPDATE table_oss SET compliance_check = \'Yes\' WHERE compliance_check = $1', [recordComplianceStatus], (err, result) => {
            if (!err) {
